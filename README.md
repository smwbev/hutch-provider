...
| `/responses` + `{"type": "x_search"}` server tool | ✅ `grok-4.5` / `grok-4.6` (citations returned as inline `url_citation` annotations) | the built-in Hermes `x_search` tool, via [hutch-x-search](https://github.com/smwbev/hutch-x-search) (override plugin, not catalog-bound) |
